...
GENEActiv.calibrate = function(binfile,
    ...
    #### 3. Find the frequency of the file. ####
    ...
    #### 4. Creating matrixes for storing output ####
    ...
    while (LD > 1) {
      ...
      AccData = try(read.bin(binfile = binfile,
      ...
      i = i + 1 # go to next block (12 hours-ish)
    } # End of While loop
    ...
    invisible(list(scale = scale,
                   offset = offset,
                   tempoffset = tempoffset,
                   cal.error.start = cal.error.start,
                   cal.error.end = cal.error.end,
                   spheredata = spheredata,
                   npoints = npoints,
                   nhoursused = nhoursused,
                   QCmessage = QCmessage,
                   use.temp = use.temp,
                   meantempcal = meantempcal,
                   bsc_qc = bsc_qc))
}
